...
# loss_fnF = F.mse_loss
# running_vloss = fit(10, modelF, loss_fnF, optF)
...
# losses[0].cpu()
# %%
# losses = map(_.cpu().numpy(), losses)
# losses_val = map(_.cpu().numpy(), losses_val)
np.save("plane_sage_01.npy", 
    {"epochs": epochs, \
    "losses": losses, \
    "losses_val": losses, \
...
modelT.load_state_dict(torch.load(pathModel,map_location=device))
modelT.eval()
