...
from modelling.modelsStore import SimpleNet, GCN0
import torch as th
...
json_file_name  = '../datasets/b2/Beam2D.json'
path_graph      = '../datasets/b2/Beam2D.adjlist'
...
for experiment in experiments:
    # break
    experiment_name = f'Beam2D_GCN0_{experiment}'
    random.seed(experiment)
    random.shuffle(graphs)

   
    train_loader = GraphDataLoader(graphs[:divede],shuffle=True, )
    test_loader = GraphDataLoader(graphs[divede:],shuffle=False, ) 
    ...
    np.save('b2/' + experiment_name + ".npy", 
        {"epochs": epochs, \
        "losses": L, \
        "losses_val": LV, \
        "time_elapsed": time_elapsed})  
    
    pathModel = 'b2/' + experiment_name + '.pt'
    ...
    plt.plot(LV) 
    plt.yscale("log")
    plt.savefig('b2/' + experiment_name + ".jpg")
    plt.close()