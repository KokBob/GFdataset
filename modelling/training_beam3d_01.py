...
for experiment in experiments:
    # break
    experiment_name = f'Beam3D_GCN0_{experiment}'
    random.seed(experiment)
    random.shuffle(graphs)

   
    train_loader = GraphDataLoader(graphs[:int(len(X0)*.7)],shuffle=True, )
    test_loader = GraphDataLoader(graphs[int(len(X0)*.7):],shuffle=False, ) 
    ...
    np.save('b3/' + experiment_name + ".npy", 
        {"epochs": epochs, \
        "losses": L, \
        "losses_val": LV, \
        "time_elapsed": time_elapsed})  
    
    pathModel = 'b3/' + experiment_name + '.pt'
    ...
    # plt.plot(losses_val) 
    plt.yscale("log")
    plt.savefig('b3/' + experiment_name + ".jpg")
    plt.close()
# %%
# for batch in test_loader:  
    # print(batch.ndata[inputs][0:5])